...
def argparser():
    ...
    parser.add_argument(
        "input_bam", type=Path,
        help="Input BAM file with tags")
    parser.add_argument(
        "output_bam", type=Path,
        help="Output BAM file with clustered UMIs")
    parser.add_argument(
        "chrom",
...
def main(args):
    ...
    with pysam.AlignmentFile(args.input_bam, "rb") as bam_in:
        # Convert BAM to dataframe
        records = []
        for read in bam_in:
            record = {
                'read_id': read.query_name,
                'CR': read.get_tag('CR') if read.has_tag('CR') else '-',
                'CB': read.get_tag('CB') if read.has_tag('CB') else '-',
                'CY': read.get_tag('CY') if read.has_tag('CY') else '-',
                'UR': read.get_tag('UR') if read.has_tag('UR') else '-',
                'UY': read.get_tag('UY') if read.has_tag('UY') else '-',
                'gene': read.get_tag('GN') if read.has_tag('GN') else '-',
                'transcript': read.get_tag('TX') if read.has_tag('TX') else '-',
                'start': read.reference_start,
                'end': read.reference_end,
                'chr': bam_in.get_reference_name(read.reference_id)
            }
            records.append(record)
        
        df_tags = pd.DataFrame.from_records(records).set_index('read_id')
    ...
    logger.info("Writing BAM output.")
    with pysam.AlignmentFile(args.input_bam, "rb") as bam_in:
        with pysam.AlignmentFile(args.output_bam, "wb", template=bam_in) as bam_out:
            for read in bam_in:
                read_id = read.query_name
                if read_id in df_tags_out.index:
                    # Update UMI tag with clustered version
                    if 'UB' in df_tags_out.columns:
                        read.set_tag('UB', df_tags_out.loc[read_id, 'UB'])
                bam_out.write(read)